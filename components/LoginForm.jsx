...
export const LoginForm = () => {
    ...
    async function handleGoogleSignin() {
        signIn('google', {callbackUrl: baseURL_DEV + '/rates' })
    }

    async function handleGithubSignin() {
        signIn('github', {callbackUrl: baseURL_DEV + '/rates' })
    }
   ...
   <>
        <div className='md:mt-20 mx-auto max-w-lg px-4 py-6'>
            <div className=" p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                ...
                {formik.errors.email && formik.touched.email ? <span className='text-rose-500 text-center text-sm -my-4 mb-1'>{formik.errors.email}</span> : <></>}
                <div className={styles.input_group}>
                    <input 
                ...
                {formik.errors.password && formik.touched.password ? <span className='text-rose-500 text-center text-sm -my-4 mb-1'>{formik.errors.password}</span> : <></>}
                {/* login buttons */}
                ...
                {
                    errorLogin ?
                    (<p className='text-rose-600 text-center italic text-sm'>{errorLogin}</p>)
                    : 
